...
public class LogUser extends javax.swing.JDialog {
    ...
    private void btnLoginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLoginActionPerformed
        ...
        if((! theSystem.fieldIsEmpty(userName))&&(! theSystem.fieldIsEmpty(userPass))){ //Revisa si los espacios estan en blanco
            if(theSystem.isCorrectUser(userName)){ //Chequea si el usuario ingresado es correcto
                User theUser = theSystem.obtainUser(userName);
                if(theUser.isCorrectPassword(userPass)){ //Chequea si su contrasenna es correcta
                    this.setVisible(false);
                    if (registrationType == 1){ //si registrationType es un 1, corresponde a un registro de audiovisuales
                        AudiovisualRegistration theWindow = new AudiovisualRegistration(new javax.swing.JFrame(), true, theSystem);
                        theWindow.setVisible(true);
                    }
                    else{ // si no es un 1, corresponde a registro de libros
